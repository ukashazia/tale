...
pub fn parse_change(action_id: ActionId, input: &str) -> Result<AdminChange, String> {
    match action_id {
        ActionId::AdminDeviceRename => Ok(AdminChange::DeviceRename {
            name: crate::admin::device_mutations::validate_machine_name(input)?,
        }),
        ActionId::AdminDeviceTagsReplace => {
            let tags = parse_values(input);
            Ok(AdminChange::DeviceTags {
                tags: crate::admin::device_mutations::canonical_tags(&tags)?,
            })
        }
        ...
        ActionId::AdminDeviceKeyExpiryConfigure => {
            let disabled = parse_bool_word(input, "key expiry")?;
            Ok(AdminChange::DeviceKeyExpiry { disabled })
        }
        ...
        ActionId::AdminRoutesReplaceApprovals => {
            let routes =
                crate::admin::route_mutations::canonical_enabled_routes(&parse_values(input))?;
            Ok(AdminChange::DeviceRoutes { routes })
        }
        ActionId::AdminDnsPreferencesEdit => Ok(AdminChange::DnsPreferences {
            magic_dns: parse_bool_word(input, "MagicDNS")?,
        }),
...
fn parse_bool_word(input: &str, field: &str) -> Result<bool, String> {
    match input.trim().to_ascii_lowercase().as_str() {
        "on" | "true" | "enabled" | "enable" => Ok(true),
        "off" | "false" | "disabled" | "disable" => Ok(false),
        _ => Err(format!("enter {field} as on/off")),
    }
}

fn parse_split_mapping(input: &str) -> Result<(String, Vec<String>), String> {
    let (domain, values) = input
        .split_once('=')
        .ok_or_else(|| "enter split DNS as suffix=resolver[,resolver...]".to_owned())?;
    let domain = crate::admin::dns_mutations::validate_domain(domain)?;
    let resolvers = crate::admin::dns_mutations::canonical_resolvers(
        &parse_values(values),
        "split-DNS resolver",
    )?;
    Ok((domain, resolvers))
